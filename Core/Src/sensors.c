...
/* 正点原子解析GPS数据——按照两个逗号之间的数据解析 */
void NMEA_GNRMC_Analysis(uint8_t *GPS_Data) {
  uint8_t *p1, dx;
  uint8_t posx;
  uint32_t temp;
  float rs;
  p1 = (uint8_t *)strstr((const char *)GPS_Data, "$GNRMC");

  posx = NMEA_Comma_Pos(p1, 1); //返回值逗号之间的数据
  if (posx != 0xFF) {           //第一个逗号得到UTC时间
    temp =
